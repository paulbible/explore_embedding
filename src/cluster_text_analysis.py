...
def calc_best_cluster_lable(vector, center_vector_map):
    dist_vals = []
    for c_label in center_vector_map:
        dist_vals.append(cosine(vector,center_vector_map[c_label]))
    return np.argmin(dist_vals)
...
def main():
    ...
    # assign variables from argument inputs
    ...
    # cluster with k mediods
    '''
    start = 10
    end = 50
    c_size = start
    for i in range(start, end+1):
        kmedoids = KMedoids(n_clusters=i, metric='cosine', random_state=0).fit(sent_mat)
        c_labels = kmedoids.labels_
        fit_val = kmedoids.inertia_
        if i == start:
            best_fit = fit_val
            c_size = start
        else:
    ...
    print(c_size, fit_val)
    #input()
    # print(len(kmedoids.labels_))
    

    kmedoids = KMedoids(n_clusters=num_clusters, metric='cosine', init='k-medoids++', random_state=0).fit(sent_mat)
    c_labels = kmedoids.labels_
    '''
    ...
    # Getting a map for each cluster label to indexes of that cluster.
    print('Getting cluster subsets')
    cluster_map = defaultdict(list)
    for i in range(len(c_labels)):
        cluster_map[c_labels[i]].append(i)

    # per cluster anlaysis
    alpha_threshold = 0.5
    threshold_map = {}
    center_vector_map = {}
    for i in range(0, len(cluster_map)):
        mark = i
        # print(cluster_map[mark])
        test_mat = sent_mat[np.array(cluster_map[mark]), :]
        print('subset', test_mat.shape)
        mean_vector = test_mat.mean(axis=0)
        center_vector_map[i] = kmedoids.cluster_centers_[i]
        centroid = kmedoids.cluster_centers_[i]
        #print(tm_vector)
        vals = []
        for row in test_mat:
            vals.append(cosine(center_vector_map[i], row))
        q = np.quantile(vals, alpha_threshold)
        print('q =', q)
        #plt.hist(vals)
        #plt.show()
        med_index = kmedoids.medoid_indices_[mark]
        print('medoid:',mark, c_labels[med_index], sentences_database[med_index])
        off_count = 0
        for i in range(sent_mat.shape[0]):
            dist = cosine(sent_mat[i], centroid)
            #if dist < q and c_labels[i] == mark:
            if dist < q:
                curr_label = c_labels[i]
                # print(c_labels[i], sentences_database[i])
                if c_labels[i] != mark:
                    off_count += 1
                    #vec = sent_mat[kmedoids.medoid_indices_[curr_label]]
                    #print(dist, cosine(sent_mat[i], vec))
        print('--> off count: ', off_count, q,alpha_threshold)
